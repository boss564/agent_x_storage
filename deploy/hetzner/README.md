...
`news_scores.jsonl` is WORM state, not a throwaway log. The hourly cron opens the file per run and closes it — regular rotation is safe once readers use `iter_jsonl_store` (`load_seen`, `load_run_markers`, `last_run_marker`). Without that, rotation would empty dedup memory and drop gate markers from the active path.
