"""JSONLogger and _safe_call for Wave 39 ethical boundary agents."""
...
class JSONLogger:
    ...
    def __init__(self, agent_name: str = "wave39", user_id: str = "default"):
        self.agent_name = agent_name
        self.user_id = user_id
        self.log_path = (
            EthicalBoundaryConfig.load().log_dir
            / f"wave39_{datetime.now(timezone.utc).strftime('%Y%m%d')}.jsonl"
        )
...
def _safe_call(
    ...
    **kwargs: Any,
    ...
    """try/except wrapper — failures surface as blocked status."""
    job_id = str(uuid.uuid4())[:12]
    start = time.monotonic()
    logger.info(f"[{node}] started", job_id=job_id)
    cfg = EthicalBoundaryConfig.load()
    ...
    for attempt in range(1, cfg.max_retries + 1):
        try:
            ...
            if attempt < cfg.max_retries:
                time.sleep(cfg.max_retries * 0.01)
    return {
        "status": "blocked",
        "job_id": job_id,
        "artifacts": [],
