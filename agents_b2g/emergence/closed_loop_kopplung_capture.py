#!/usr/bin/env python3
"""Closed-loop coupling capture — CLOSED_LOOP_KOPPLUNG_v0 (BINDEND).

φ_L + R_ij recursion · Arm B: h* from sticky M · Arm C: delivery on M,
coupling reads R/h under π(M). Fixed η=1.0 (F1). Battery A∧B∧C precondition.
...
def capture_closed_loop_coupling(
    *,
    cycles: int = 512,
    warmup_ticks: int = 32,
    run_seed: int = 20261601,
    kappa: float = 0.0,
    arm: str = "B",
) -> Dict[str, Any]:
    """Return {trace, precondition, battery, arm, kappa, eta}."""
    ...
    tmp_root = Path(
        os.environ.get(
            ...
            str(Path(os.environ.get("TMPDIR", "/tmp")) / "emergence_finale_cl_kopplung"),
        )
    )
    orch = FinaleOrchestrator(
        user_id="emergence-cl-kopplung",
    ...
    for _ in range(total):
        ...
        if tick > warmup:
            # γ-update always from true ledger edges (delivery map)
