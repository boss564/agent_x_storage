#!/usr/bin/env python3
"""Minimal κ=0 ledger capture exporting sticky map + avg_latency hist for S_i probe."""
    ...
    *,
    ...
    tmp_root = Path(
        os.environ.get(
            ...
            str(Path(os.environ.get("TMPDIR", "/tmp")) / "emergence_finale_s_probe"),
        )
    )
    orch = FinaleOrchestrator(
        user_id="emergence-s-probe",
    ...
    for _ in range(warmup + int(cycles)):
        tc.cycle += 1
        tick = int(tc.cycle)
