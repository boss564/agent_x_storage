"""JSONLogger and _safe_call for Post-MEV diagnostic agents."""
...
class JSONLogger:
    ...
    def __init__(self, agent_name: str = "post_mev", user_id: str = "default"):
        self.agent_name = agent_name
        self.user_id = user_id
        self.log_path = (
            PostMEVConfig.load().log_dir
            / f"post_mev_{datetime.now(timezone.utc).strftime('%Y%m%d')}.jsonl"
        )
    ...
    **kwargs: Any,
    ...
    for attempt in range(1, cfg.max_retries + 1):
        try:
            ...
            last = exc
            logger.error(f"[{node}] attempt {attempt} failed", job_id=job_id, error=str(exc))
