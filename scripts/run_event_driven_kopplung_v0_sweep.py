#!/usr/bin/env python3
"""κ-Sweep — EVENT_DRIVEN_KOPPLUNG_v0 (BINDEND).

docs/EVENT_DRIVEN_KOPPLUNG_v0_PREREG.md
F5 Inter-Arrival · F6 Snapshot Δt=64 · Seeds 20262001…06
"""
...
from event_driven_kopplung_capture import capture_event_driven_coupling  # noqa: E402
from measure import divergence, kuramoto  # noqa: E402

KAPPAS = [0.0, 0.2, 0.4, 0.6, 0.8, 1.2]
RUN_SEEDS = [20262001, 20262002, 20262003, 20262004, 20262005, 20262006]
SPOT_SEED = 20262001
LOCKED_MAX_SEED = 20261999
ALPHA = 0.05
N_SURROGATES = 200
DELTA_R_MIN = 0.10
R_FLOOR = 0.34
...
def run_cell(arm: str, kappa: float, run_seed: int) -> dict:
    ...
    pack = capture_event_driven_coupling(
        run_seed=run_seed, kappa=kappa, arm=arm,
    )
    tr = pack["trace"]
...
def evaluate(cells: list[dict]) -> dict:
    ...
    for kappa in KAPPAS:
        ...
        for seed in RUN_SEEDS:
            ...
            if b_ok:
                intact_n += 1
            gate = bool(
                b_ok and c and gate_seed(b["r"], b["p"], b["d_dyn"], c["r"])
            )
            if gate:
                gate_passes += 1
            c_coup = bool(
                b_ok and c and c.get("coupling_assess") == "COUPLED"
    ...
    form_ok, form_meta = (
        form_criterion(form_r, sd_pool) if len(form_r) >= 2
        else (False, {"note": "need ≥2 intact κ"})
    )
    ...
    if c_coupled_any:
        ...
        d_b_star = [
            c["d_dyn"] for c in by("B", kappa_star)
...
def write_md(path, eval_res, elapsed, spot, signal_blind, json_path):
    ...
    lines += [
        ...
        "",
        "Serie (Tick-Kopplung) bleibt versiegelt. Keine Schwellen-Nachjustierung.",
        "",
    ]
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=_PROJECT_ROOT / "agents_b2g" / "emergence" / "event_driven_kopplung_v0",
    )
    args = ap.parse_args()
    ...
    print("=" * 60)
    print("EVENT_DRIVEN_KOPPLUNG_v0 (BINDEND · F5 Inter-Arrival · F6 Δt=64)")
    ...
    print(
        ...
        f"|ΔΔR|={spot['mean_abs_diff']} snaps={spot['n_snapshots']}"
    )
    ...
    if signal_blind:
        ...
        (out_dir / "EVENT_DRIVEN_KOPPLUNG_ERGEBNIS.md").write_text(
    ...
    for arm in ("B", "C"):
        ...
        for kappa in KAPPAS:
            for seed in RUN_SEEDS:
                ...
                print(
                    f"    r={c['r']} p={c['p']} "
                    f"precon={c['precondition_label']} assess={c['assess_verdict']}"
                )
    ...
    json_path = out_dir / "EVENT_DRIVEN_KOPPLUNG_FULL.json"
    payload = {
        "pre_reg": "docs/EVENT_DRIVEN_KOPPLUNG_v0_PREREG.md",
        "status": "BINDEND",
        "f5": "inter_arrival",
        "f6_snapshot_dt": 64,
        "spot_check": spot,
    ...
    md_path = out_dir / "EVENT_DRIVEN_KOPPLUNG_ERGEBNIS.md"
