#!/usr/bin/env python3
"""κ-Sweep — CLOSED_LOOP_KOPPLUNG_v0 (BINDEND).

docs/CLOSED_LOOP_KOPPLUNG_v0_PREREG.md
η=1.0 · Per-κ Batterie A∧B∧C · PRECONDITION_LOST
...
from closed_loop_kopplung_capture import capture_closed_loop_coupling  # noqa: E402
from measure import divergence, kuramoto  # noqa: E402

KAPPAS = [0.0, 0.2, 0.4, 0.6, 0.8, 1.2]
RUN_SEEDS = [20261601, 20261602, 20261603, 20261604, 20261605, 20261606]
SPOT_SEED = 20261601
SCREENING_SEEDS = {20261501, 20261502, 20261503}
ALPHA = 0.05
N_SURROGATES = 200
DELTA_R_MIN = 0.10
R_FLOOR = 0.34
...
def run_cell(
    ...
    if run_seed in SCREENING_SEEDS:
        raise RuntimeError(f"HARKing: screening seed {run_seed} gesperrt")
    pack = capture_closed_loop_coupling(
        cycles=cycles,
        warmup_ticks=warmup,
        run_seed=run_seed,
        kappa=kappa,
...
def evaluate(cells: list[dict]) -> dict:
    ...
    if c_coupled_any:
        ...
        if lost_all:
            ...
            reason = "no κ-stage with ≥4/6 intact battery A∧B∧C"
        else:
            verdict = "NO_COUPLING"
...
def write_md(
    ...
    lines = [
        ...
        f"**JSON:** `{json_path}`",
        f"**η:** {ETA} (F1) · Seeds `{RUN_SEEDS[0]}…{RUN_SEEDS[-1]}`",
        "",
    ]
    if spot is not None:
        lines += [
            ...
            "",
            f"- Batterie intact: **{spot.get('precondition_intact')}** "
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=_PROJECT_ROOT / "agents_b2g" / "emergence" / "closed_loop_kopplung_v0",
    )
    args = ap.parse_args()
    ...
    print("=" * 60)
    print("CLOSED_LOOP_KOPPLUNG_v0 κ-Sweep (BINDEND · η=1.0 · Batterie)")
    ...
    if signal_blind:
        print("SIGNAL_BLIND: Spot-Check Batterie failed — Sweep gesperrt.")
        md = out_dir / (
            "CLOSED_LOOP_KOPPLUNG_ERGEBNIS_SMOKE.md"
            if args.smoke
            else "CLOSED_LOOP_KOPPLUNG_ERGEBNIS.md"
        )
        md.write_text(
            "\n".join([
                "# CLOSED_LOOP_KOPPLUNG_v0 — SIGNAL_BLIND",
                "",
                f"**Spot κ=0 Seed {SPOT_SEED}:** Batterie A∧B∧C **FAIL**",
                f"- A={spot['battery_A']} ρ={spot['median_abs_rho']}",
                f"- B={spot['battery_B']} mae_n={spot['mae_norm']}",
                f"- C={spot['battery_C']} |ΔΔR|={spot['mean_abs_diff']}",
                "",
                "Verdict: `SIGNAL_BLIND` — Sweep nicht ausgeführt.",
                "",
            ]),
            encoding="utf-8",
        )
    ...
    for seed in RUN_SEEDS:
        ...
        print(
            ...
            f"A={c['battery_A']} B={c['battery_B']} C={c['battery_C']}"
        )
    ...
    json_path = out_dir / f"CLOSED_LOOP_KOPPLUNG_{tag}.json"
    md_path = out_dir / (
        "CLOSED_LOOP_KOPPLUNG_ERGEBNIS_SMOKE.md"
        if args.smoke
        else "CLOSED_LOOP_KOPPLUNG_ERGEBNIS.md"
    ...
    payload = {
        "pre_reg": "docs/CLOSED_LOOP_KOPPLUNG_v0_PREREG.md",
        "status": "BINDEND",
        "eta": ETA,
        ...
        "constants": {
            "kappas": KAPPAS,
            "run_seeds": RUN_SEEDS,
            "screening_seeds_locked": sorted(SCREENING_SEEDS),
            "warmup": warmup,
            "cycles": cycles,
            "alpha": ALPHA,
    ...
    write_md(md_path, eval_res, elapsed, args.smoke, json_path, spot, signal_blind)

    print("\n" + "=" * 60)
    print(f"VERDICT: {eval_res['verdict']}")
    print(f"  {eval_res['reason']}")
    print(f"elapsed: {elapsed:.0f}s → {md_path}")
    print("=" * 60)
    return 0
