"""Schicht B: LitePSM gem() / Classic gemJoin() / CCTP localMessageTransmitter()."""
...
SEL_GEM = "0x" + keccak(b"gem()")[:4].hex()
SEL_GEM_JOIN = "0x" + keccak(b"gemJoin()")[:4].hex()
SEL_LOCAL_MT = "0x" + keccak(b"localMessageTransmitter()")[:4].hex()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Resolve Stablecoin Mint/Burn contracts")
    parser.add_argument(
        "--candidates",
    ...
    for item in body.get("contracts", []):
        ...
        try:
            ...
            if protocol == "lite_psm":
                raw = jsonrpc(rpc, "eth_call", [{"to": addr, "data": SEL_GEM}, "latest"])
                gem = decode_address(raw)
                extra = {"gem": gem}
                ok = gem.lower() == USDC_ETH
            elif protocol == "classic_psm":
                raw = jsonrpc(rpc, "eth_call", [{"to": addr, "data": SEL_GEM_JOIN}, "latest"])
                gem_join = decode_address(raw)
                extra = {"gem_join": gem_join}
                ok = int(gem_join, 16) != 0
            elif protocol in ("cctp_v1", "cctp_v2"):
                raw = jsonrpc(rpc, "eth_call", [{"to": addr, "data": SEL_LOCAL_MT}, "latest"])
                mt = decode_address(raw)
                extra = {"local_message_transmitter": mt}
                ok = int(mt, 16) != 0
            else:
